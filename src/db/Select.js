...
async function AwaitSelect(dbId, query, values, cache) {
    ...
    return await ExecuteQuery(global.QueryTypes.Select, data.dbId, data.query, data.values, null, data.cache);
}
...
async function Select(dbId, query, values, callback, cache) {
    ...
    await ExecuteQuery(global.QueryTypes.Select, data.dbId, data.query, data.values, data.callback, data.cache);
}
