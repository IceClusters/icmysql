...
async function AwaitInsert(dbId, query, values) {
    ...
    return await ExecuteQuery(invokingResource, QueryTypes.Insert, data.dbId, data.query, data.values, null, false);
}
...
async function Insert(dbId, query, values, callback) {
    ...
    const invokingResource = GetInvokingResource();
    await ExecuteQuery(invokingResource, QueryTypes.Insert, data.dbId, data.query, data.values, data.callback, false);
}
