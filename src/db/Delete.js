...
async function AwaitDelete(dbId, query, values) {
    ...
    return await ExecuteQuery(QueryTypes.Delete, data.dbId, data.query, data.values, null, data.cache);
}
...
async function Delete(dbId, query, values, callback) {
    ...
    await ExecuteQuery(QueryTypes.Delete, data.dbId, data.query, data.values, data.callback, data.cache);
}
